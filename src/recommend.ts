...
import { buildLibIndexFrom, db, inLibrary } from './db';
import {
...
async function build(): Promise<RecSection[]> {
  ...
  const libIndex = buildLibIndexFrom(shows, movies);
  const isKnown = (r: Rec) => inLibrary(libIndex, r.kind, r.tmdbId, r.name, r.year);
